...
import com.trs.financial.pojo.vo.GoodwillRatioVO;
...
/**
 * @Description 二、资产负债表
 * 第9步(2)：看商誉，了解公司未来业绩爆雷的风险
 * @Author lu.ziyong
 * @Date 2021/8/11
 * @Version 1.0
 */
public class GoodwillRatioPolicy extends LoopColumnTableRenderPolicy {

    @Override
    public void render(ElementTemplate eleTemplate, Object data, XWPFTemplate template) {
        if (!eleTemplate.getSource().equals("{{GoodwillRatioPolicy}}"))
        ...
        try {
            ...
            ArrayList<GoodwillRatioVO> vos = new ArrayList<>();
            for (int i = 0; i < 5; i++) {
                IWenCaiDataDTO dto = dtos.get(i);
                IWenCaiDataDTO dto2 = dtos2.get(i);

                GoodwillRatioVO vo = new GoodwillRatioVO();
                vo.setYear(dto.getDate());
                vos.add(vo);
            }
            super.render(eleTemplate, vos, template);
        } catch (Exception e) {
            throw new PolicyException("第9步(2)：看商誉，了解公司未来业绩爆雷的风险，分析异常", e);
        }
    }
}